...
class compute_RDF():
    ...
    def water_water(topology_file, dcd_file, save_location, ligand_selection="resname UNK"):
        ...
        # Calculate RDF
        rdf = RDF.InterRDF(water, water, nbins=100, range=(0, 15.0),
                           exclusion_block=(1, 1))  # avoid computation to itself
        rdf.run()
        ...
        plt.ylabel('RDF')
        plt.title(
            f'Radial Distribution Function of water-water\nfor {dcd_file}')
    ...
    def water_atom(topology_file, dcd_file, save_location, ligand_selection="resname UNK"):
        ...
        atoms = [universe.select_atoms(f"index {atom.index} and {ligand_selection}")
        ...
        plt.ylabel('RDF')
        plt.title(
            f'Radial Distribution Function of water-atom\nfor {dcd_file}')
    ...
    def single_water_single_atom(topology_file, dcd_file, save_folder=None, ligand_selection="resname UNK"):
        ...
        # Select atoms for which you want to calculate RDF
        water = universe.select_atoms(f"type O and not {ligand_selection}")
        ags = [[universe.select_atoms(f"index {atom.index} and {ligand_selection}"), water]
               for atom in universe.select_atoms(ligand_selection)]
        ss_rdf = RDF.InterRDF_s(universe, ags, nbins=100, range=(0, 15.0),
                                norm="density", density=True)  # final RDF is over the average density of the selected atoms in the trajectory box
        ss_rdf.run()
...
def plot_dihedrals(topology_file, dcd_file, save_folder=None, ligand_selection="resname UNK"):
    ...
    for idx, dihedral in enumerate(universe.dihedrals):
        ...
        plt.hist(dihedral_angles[:, idx], bins=50,
    ...
    for idx, dihedral in enumerate(universe.dihedrals):
        ...
        plt.hist(dihedral_data, bins=50, density=True,
                 alpha=0.7, color=legend_colors[label])
...
class plot_histograms():
    ...
    def plot_potential_energy(txt_file, save_location):
        ...
        plot_histograms._plot_stdout_histogram_base(txt_file, save_location,
    ...
    def plot_kinetic_energy(txt_file, save_location):  # using the output file density
        ...
        plot_histograms._plot_stdout_histogram_base(txt_file, save_location,
                                                    ...
                                                    prop="Kinetic Energy")  # mL is cm^3

    @staticmethod
    def plot_density(txt_file, save_location):  # using the output file density

        plot_histograms._plot_stdout_histogram_base(txt_file, save_location,
                                                    data_name="Density (g/mL)",
                                                    prop="Water Density")  # mL is cm^3
...
class plot_energies():
    ...
    def plot_potential_energy(txt_file, save_location):
        plot_energies._plot_stdout_with_time_base(
            txt_file, save_location, data_name="Potential Energy (kJ/mole)", prop="Potential Energy")

    @staticmethod
    def plot_kinetic_energy(txt_file, save_location):
        plot_energies._plot_stdout_with_time_base(
            txt_file, save_location, data_name="Kinetic Energy (kJ/mole)", prop="Kinetic Energy")

    @staticmethod
    def plot_energy_all_windows(folder, save_location, prop="Kinetic Energy", base_name="stdout_prod_", no_windows=11):

        prop_dict = {"Kinetic Energy": "Kinetic Energy (kJ/mole)",
        ...
        for w in range(0, 11, 1):
            ...
            plt.plot(x_axis, data, linewidth=2, alpha=0.3,
                     label=label, color=colors[w])
            legend_labels.append(label)
            plt.xlabel(f'Steps')
            plt.ylabel(f'{data_name}')
            plt.title(f'Plot of {prop} for {txt_file}')
