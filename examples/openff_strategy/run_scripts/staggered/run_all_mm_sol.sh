...
#mobley_3053621 c1ccccc1
ligand_array=( "mobley_242480" )
smiles_array=( "c1ccc2c(c1)C(=O)c3c(ccc(c3C2=O)N)N" )
...
if [[ ! -d $ligand/${scheme_array[i]}/mm_sol ]]; then
    mkdir $ligand/${scheme_array[i]}/mm_sol
fi

arguments=( --smiles $smiles --folder $ligand/${scheme_array[i]}/mm_sol )
...
#fi
arguments+=( --ligand-forcefield None --windows 16 --sampling 6 )
