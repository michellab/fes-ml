...
if [[ ! -d $ligand/${scheme_array[i]}/concerted ]]; then
    mkdir $ligand/${scheme_array[i]}/concerted
fi

arguments=( --smiles $smiles --folder $ligand/${scheme_array[i]}/concerted )
...
#fi
arguments+=( --windows $NUM_WINDOWS --sampling 6 --ml-application correction --run_window $w )
